...
a(Parent) ->
    {ok, Pid} = trade_fsm:start_link("Jakub"),
    Parent ! Pid,
    io:format("Spawned Jakub: ~p~n", [Pid]),
    %sys:trace(Pid,true),
    timer:sleep(800),
...
b(PidA, PidCliA) ->
    {ok, Pid} = trade_fsm:start_link("Czeslaw:"),
	spawn(fun() -> controller(PidA,Pid) end),
    io:format("Spawned Czeslaw: ~p~n", [Pid]),
    %sys:trace(Pid,true),
    timer:sleep(500),
...
controller(PidA,PidB) ->
	 io:format("Controller is working"),
	timer:sleep(2000),
	delete(PidA,PidB,"boots"),
	io:format("Controller : deleting boots ! ~n").

	

delete(OwnPid,OtherPid,Item) ->
	gen_fsm:send_event(OtherPid, {controller_delete_own, Item}),
	gen_fsm:send_event(OwnPid, {controller_delete_others, Item}).
